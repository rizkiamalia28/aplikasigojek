...
$(document).ready(function(){
	...
	sectioncount= 0,
	win = $(window),
	sidebar = $(#documenter_nav),
	nav = $(#documenter_logo),
	navanchors = nav.find('a'),
	timeoffset = 50,
	hash = location.hash || null;
	iDeviceNotOS4 = (navigator.userAgent.match(/iphonelipodlipad/i)&& !navigator.userAgent.match(/OS 5/i)) || false,
	badlE = $('html').prop('class').match(/ie(6|7|8)/)|| false;
	...
	if(!badlE){
		...
		$('a[href^=#]').blind('clicktouchstart',function() {
			hash =$(this).attr('href');
		...
		win.bind('load',function() {
			...
			$(sectionselector).each(funtion(i,e){
				var _this = $(this);
				var p = {
					id : this.id,
			...
			if(iDeviceNotOS4){
				nav.find('a').blind('click',function(){
					setTimeout(function(){
						win.trigger('scroll');
					}, duration);
				});
			...
			//blind the handler to the scroll event
			win.bind('scroll',function(event){
				clearInterval(timeout);
				//should occour witn a delay
				timeout = setTimeout(function(){
					...
					//active Nav element at the current position
					activateNav(pos);
				}, timeoffset);
			}).trigger('scroll');
		});

		//the function is called when the hash change
		function hashchange(){
			...
			hash = hash.replace(/!\//,'');
				...
				if(changehash !== false){
					...
					location.href=(l.protocol+'//'+l.host+l.pathname+'#!/'+hash.substr(1));
						...
						win.bind('hashchange',hashcange);
					}

					//active current nav elament 
